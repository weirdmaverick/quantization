...
def batch_nll_and_count(logits: np.ndarray, ids: np.ndarray, mask: np.ndarray):
    ...
    # Per-token log-prob of the true label
...
# ───────────────────────────────────────────────────────────────
# evaluation function (time, size, profile, perplexity return)
# ───────────────────────────────────────────────────────────────
def evaluate(onnx_dir: str, ds, max_length: int, output_json: str):
    ...
    for ex in tqdm(ds, desc=f"[Evaluate] {os.path.basename(onnx_dir)}"):
        ...
        logits = outputs[0]  # (1, L, V)
        nll, ntoks = batch_nll_and_count(logits, feed["input_ids"], feed["attention_mask"])
        total_nll    += nll
        total_tokens += ntoks
    ...
    return total_t, size_mb, prof_path, ppl
...
if __name__ == "__main__":
    ...
    ds_raw = load_dataset(DATASET[0], DATASET[1], split=DATASET[2])
    def preprocess_batch(batch):
        tok = tokenizer(batch["text"], truncation=True, padding="max_length", max_length=max_length)
        seq_len = len(tok["input_ids"][0])
        tok["position_ids"] = [list(range(seq_len))] * len(tok["input_ids"])
        return tok

    ds_tok = ds_raw.map(preprocess_batch, batched=True, remove_columns=ds_raw.column_names)
    ...
    # 3) FP32 Evaluation
    t_fp32, s_fp32, prof_fp32, ppl_fp32 = evaluate(
        ONNX_FP32_DIR, ds_tok, max_length, f"fp32_{output_json}"
    )
    print(f"FP32 → time: {t_fp32:.2f}s | size: {s_fp32:.2f}MB | ppl: {ppl_fp32:.2f}")

    # 4) INT8 Evaluation
    t_int8, s_int8, prof_int8, ppl_int8 = evaluate(
        ONNX_INT8_DIR, ds_tok, max_length, f"int8_{output_json}"
    )
    print(f"INT8 → time: {t_int8:.2f}s | size: {s_int8:.2f}MB | ppl: {ppl_int8:.2f}")

    # 5) Result
    print(f"\n[Result] Size↓: {(s_fp32 - s_int8)/s_fp32*100:.1f}%  Latency↑: {(t_fp32 - t_int8)/t_fp32*100:.1f}%  PPL↑: {(ppl_int8 - ppl_fp32)/ppl_fp32*100:.1f}%")
    sys.exit(0)
